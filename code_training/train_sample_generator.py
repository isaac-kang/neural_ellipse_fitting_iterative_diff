    ...
    def generate_data(self, imgfilename1, imgfilename2):
        ...
        if self.noise:
            if np.random.randint(3) == 0:
                #   Blur
                blur_radius = int((np.random.normal() * 3.0) ** 2 + 0.5) * 2 + 1
                if blur_radius > 0:
        ...
        # dict of image, mask, center, anno_cetner
        return dict, True
...
"""
def iterate_data_directory(datadirs):
    input_image_size = IMAGE_WIDTH
    face_diag_size = (384 * input_image_size) // 512
    landmark_data_type = 'int32'
    dataset = iterateTrainData(datadirs, input_image_size, face_diag_size )
"""


def show_samples(img_directory, mask_directory):
    tic = time.time()
    data_generator = TrainDataGenerator(img_directory, mask_directory)
    print(f'elapsed = {time.time()-tic}')

    mygenerator = data_generator()
    ...
    while True:
        ...
        for i in range(ny * nx):
            data_dict = mygenerator.__next__()

            canvas = data_dict['image']
            mask = data_dict["mask"]
            center = data_dict['mask_center']
            anno_center = data_dict['annotated_center']

            canvas = ((canvas) / np.sqrt(2.0) + 0.5)
            ...
            x = np.int(center[0])
            y = np.int(center[1])
            mask_with_center = cv2.circle(mask, (x, y), 10, (1, 0, 0), thickness=3)
            x = np.int(anno_center[0])
            y = np.int(anno_center[1])
            mask_with_center = cv2.circle(mask_with_center, (x, y), 15, (0, 1, 0), thickness=3)

            img_samples.append(canvas)
            mask_samples.append(mask_with_center * 0.9 + canvas * 0.1)
        ...
        mask_samples = mask_samples.reshape(ny, nx, height, width, channels).transpose(0, 2, 1, 3, 4).reshape(height * ny, width * nx, channels)

        cv2.imshow("img", np.concatenate([img_samples[:, :, ::-1], mask_samples[:, :, ::-1]], axis=1))
        key = cv2.waitKey(0)
        if key == ord('q'):
            break


def data_statistics(datadir, maskdir, num_samples=100):
    data_generator = TrainDataGenerator(datadir, maskdir)
    ...
    for i in range(num_samples):
        ...
        for name in DATA_FIELD_NAMES:
            batch_dict[name].append(data_dict[name])

    for name in DATA_FIELD_NAMES:
        print("Statistics about ", name)

        batch_dict[name] = np.array(batch_dict[name])
        print('\t' + 'mean =', np.mean(batch_dict[name]))
        print('\t' + 'min =', np.min(batch_dict[name]))
        print('\t' + 'max =', np.max(batch_dict[name]))
        print('\t' + 'stddev =', np.std(batch_dict[name]))
...
if __name__ == "__main__":
    ...
    img_directory = ['/home/hikoo/project/ssd/iris_pupil/pupil_labs/gray_without_blink', '/home/hikoo/project/ssd/iris_pupil/ExCuSe/ExCuSe_Origin']
    mask_directory = ['/home/hikoo/project/ssd/iris_pupil/pupil_labs/pupil_ellipse_without_blink', '/home/hikoo/project/ssd/iris_pupil/ExCuSe/Seg/']

    data_statistics(img_directory, mask_directory)

    if True:
        show_samples(img_directory, mask_directory)
