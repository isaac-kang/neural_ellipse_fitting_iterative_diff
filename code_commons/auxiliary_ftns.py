...
def gaussian_heatmap(joints, sigma=8):
    num_joints = len(joints)
    image_size = np.array([IMAGE_HEIGHT, IMAGE_WIDTH])
    heatmap_size = np.array([IMAGE_HEIGHT, IMAGE_WIDTH])
    target = np.zeros((num_joints, heatmap_size[1], heatmap_size[0]), dtype=np.float32)
    tmp_size = 3 * sigma

    target_weight = np.ones((num_joints, 1), dtype=np.float32) #(1: visible, 0: invisible)
